...
ax.set_xlabel("Datum")
ax.plot(prediction, label="Sensor mit ML-kalibrierten Daten", color = "coral")
ax.plot(futuredata_lubw.index, futuredata_sont_c["alphasense_cali"], label="Herstellerkalibrierung", color = "limegreen")
ax.plot(futuredata_lubw.data_NO2, label="LUBW", color = "black")
ax.set_ylabel("NO2-Konzentrationen [μg/m^3]")
ax2 = ax.twinx()
ax2.set_ylabel("T in °C, RH in %")
ax2.plot(futuredata_lubw.data_sht_humid, linestyle='--', color="dodgerblue", label = "RH")
ax2.plot(futuredata_lubw.data_sht_temp, color = "gold", label = "T")
ax.grid(True)
ax.legend()
